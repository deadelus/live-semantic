...
- [x] Corrigé le 2026-08-09, trouvé via instrumentation en usage réel (`logs/livesemantic.log`, `internal/application/uc/uc_recognition.go` log `"Frame timing"`) : `gocvtracker.Tracker.Init`/`Update` reconvertissait `image.Image → gocv.Mat` (`gocv.ImageToMatRGB`, boucle pixel par pixel pure Go, ~100ms mesuré sur 720p) **indépendamment pour chaque track actif**, alors que `trackManager` passe le même `*entities.Frame` à tous les trackers d'un même tick. `detect_or_track_ms` scalait quasi-linéairement avec `active_tracks` (~100ms à 1 track, ~500ms à 3) sur les frames `advance`, censées être bon marché. Fix : cache par identité de pointeur `*entities.Frame` dans `gocv-tracker` (`sharedFrameMat`), safe car boucle vidéo mono-thread (§ C pas encore async). Pas re-mesuré en conditions réelles (pas de webcam ici) — à confirmer par qui a la machine.
