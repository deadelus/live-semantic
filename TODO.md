...
- [ ] Intégrer un tracker mono-objet gocv (**KCF, CSRT ou MIL** — MOSSE corrigé : absent du binding `gocv v0.42.0` utilisé ici, voir `docs/adr/object-tracking.md`. À trancher selon un test de dérive sur vidéo réelle, aucun des trois n'est présupposé meilleur). **Dépendance : `infrastructure/tracking.ObjectTracker` (E, fait). Effort : M (0.5-1 j, gocv expose KCF/CSRT nativement via `gocv.io/x/gocv/contrib` — module `opencv_tracking` confirmé présent dans la formule Homebrew `opencv` utilisée par ce projet sur macOS, Linux non vérifié — donc pas de portage à faire, juste l'intégration + tests de dérive).**
