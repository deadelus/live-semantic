...
- [x] Renommer `infrastructure/ai.AI` en `infrastructure/inference.ObjectDetector` (fait le 2026-08-05, avant même le reste de la phase E — l'occasion s'est présentée en réorganisant `implementation/ai` → `implementation/inference/onnx/{runtime,yolo11s}`). **Reste à faire** : `DetectionResult` retourne toujours `[]onnx.BoundingBox` (type vendor) au lieu de `[]DetectedObject{Class, BBox, Score}` — c'est la vraie fuite d'abstraction, voir item suivant.
