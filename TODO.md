...
- [ ] **Non résolu, en cours d'investigation** : `KCF/CSRT backend.Update()` (l'appel OpenCV natif lui-même, isolé par instrumentation temporaire dans `gocv-tracker`) prend 10-20ms dans `cmd/tracking-drift` (fichier vidéo) mais 200-500ms sur webcam réelle dans `cmd/livesemantic` — jusqu'à x25, reproductible mais pas strictement corrélé au nombre de tracks actifs (vu aussi élevé avec 1 seul track sur certains runs). Essayé et **infirmé** : `gocv.SetNumThreads(1)` (aucun effet mesuré). Piste suivante non testée : `VECLIB_MAXIMUM_THREADS=1` (Accelerate/vDSP macOS, si le FFT interne de KCF/CSRT passe par là plutôt que par `cv::parallel_for_`). Si ça échoue aussi : nécessite un vrai profiler (Instruments Time Profiler) sur la machine réelle, pas exploitable en aveugle depuis des chronos externes. **Effet de bord positif trouvé en parallèle** : le filtre de classe (`req.Filter`) n'était appliqué qu'à l'alerte, jamais au tracking lui-même — tout objet détecté était suivi/dessiné (bug pré-existant, jamais un vrai filtre). Corrigé (`trackManager.reanchor`), réduit mécaniquement le nombre de tracks concurrents en usage normal, donc potentiellement une partie du problème perçu — sans expliquer la totalité de l'écart x25 mesuré isolément.
